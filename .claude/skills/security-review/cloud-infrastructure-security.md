...
description: クラウドプラットフォーム（AWS/Vercel/Railway/Cloudflare）へのデプロイ、IAM ポリシーの管理、CI/CDパイプラインの実装、ログ/モニタリング設定、Infrastructure as Code 作業時にこのスキルを使用する。IAM最小権限・シークレットローテーション・ネットワーク隔離・ログ監査・WAF・バックアップの 7 領域のチェックリストとコード例を提供。security-review がアプリケーション層、本スキルがインフラ層を扱う。
---
